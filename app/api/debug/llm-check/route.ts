/**
 * TEMPORARY diagnostic endpoint — remove after verifying the LLM provider.
 *
 * GET /api/debug/llm-check?token=llmdiag-4b8e2c
 *
 * Runs INSIDE the deployed Container App (real production env). Reports which OpenAI
 * env vars are present (never the key value), the resolved config (incl. the exact
 * deployment URL), and the result of a trivial live completion — success text or the
 * exact provider error (status + snippet). Token-gated; never returns the API key.
 */
...
export async function GET(request: Request): Promise<Response> {
  ...
  const present = {
    ...
    AZURE_OPENAI_DEPLOYMENT_NAME: env.AZURE_OPENAI_DEPLOYMENT_NAME ?? null,
  ...
  if (!config) {
    return NextResponse.json({ step: 'readOpenAiConfig', configured: false, present, note: 'null → OPENAI_API_KEY not visible at runtime' })
  }

  const resolved = {
    isAzure: config.isAzure,
    baseUrl_host: host(config.baseUrl),
    resolved_deployment: config.model, // ← the deployment name the app will call
